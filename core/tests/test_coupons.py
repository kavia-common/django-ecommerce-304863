...
def test_admin_coupon_crud(api_client, auth_headers_for_admin):
    ...
    detail = api_client.get(
        f"/api/admin/coupons/{coupon_id}/", **auth_headers_for_admin
    )
...
@pytest.mark.django_db
def test_coupon_apply_to_order_is_idempotent(
    coupon_factory, item_factory, order_factory, user
):
    ...
    ok1, msg1, red1 = coupon.apply_to_order(
        user=user, order=order, idempotency_key="k1"
    )
    assert ok1 is True
    assert red1 is None
    ...
    ok2, msg2, red2 = coupon.apply_to_order(
        user=user, order=order, idempotency_key="k1"
    )
    assert ok2 is True
    assert red2 is None
...
@pytest.mark.django_db
def test_template_add_coupon_view_applies_coupon(
    django_client, coupon_factory, item_factory, order_factory, user
):
