...
def test_inventory_release_reservation_idempotent(item_factory, order_factory, user):
    ...
    rel1 = release_inventory_reservations_for_order(order=order, performed_by=user, idempotency_key="rel", reason="x")
    ...
    rel2 = release_inventory_reservations_for_order(order=order, performed_by=user, idempotency_key="rel", reason="x")
